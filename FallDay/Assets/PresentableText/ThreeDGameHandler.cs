...
public partial class ThreeDGameHandler : MonoBehaviour
{
    ...
    public void CatchDirection(SwipeDirection direction)
    {
        ...
        if(debugbool) Debug2DGrid(RoomsInternalGrid, "MainGrid Moved");
    }
            ...
            for (int y = 0; y < RoomsInternalGrid.GetLength(1); y++)
            {
                if (RoomsInternalGrid[x, y] != null)
                {
                    ...
                    else
                    {
                        ...
                        if(debugbool) Debug.LogWarning($"Room {RoomsInternalGrid[x, y].name} at ({x}, {y}) moved out of bounds to ({newX}, {newY}). Not adding to new Array.");
                        Destroy(RoomsInternalGrid[x, y]);
                    }
                }
            }
        }
        if (debugbool) Debug2DGrid(ToReturn, "MovedGrid");
        return ToReturn;
    ...
    private void SetRoomInInternalGrid(GameObject Room, DoorHandler.DoorDirection doorDirectionIndex) // Sets the current grid to the new grid generated by GenerateMovedGrid()
    {
        int pos = ((int)doorDirectionIndex);
    ...
    private bool SheckForOpenSpaceInGrid(DoorHandler.DoorDirection doorDirectionIndex) // Sets the current grid to the new grid generated by GenerateMovedGrid()
    {
        int pos = ((int)doorDirectionIndex);
        // spot Open = true. spot ocuppied false;
        bool returnValhue = false;
        switch (PlayerDirection) // Same switch condition as MovedGrid
        {
            default: // generate to Move rooms north or Up // if pos is not ocupyed by a room
                //if (debugbool) Debug.LogWarning("Player Direction is None Or Does not Matsh the SetNewRoomsInGridSwitch, no movement will occur.");
    ...
    private void CreateThreeWay(GameObject rootRoom)
    {
        ...
        foreach (var room in rooms)
        {
            ...
            if (SheckForOpenSpaceInGrid((DoorHandler.DoorDirection)doorDirectionIndex))
            {
                var newlyCreatedRoomPrefab = 
                    Instantiate(rooms[roomIndexer].roomPrefab,
                                GetRoomsSpawnDiff(rootRoom,
                ...
                roomIndexer++;
                SetRoomInInternalGrid(newlyCreatedRoomPrefab, (DoorHandler.DoorDirection)doorDirectionIndex);
            }
            else { roomIndexer++; }
            doorDirectionIndex++;
        }
        if (debugbool) Debug.Log($"Create three way with {rootRoom.name} as the base!");
        if (debugbool) Debug2DGrid(RoomsInternalGrid, "MainGrid");
    }
    ...
    //      0.1 | 1.1 | 2.1 | 3.1 | 4.1
    //      0.0 | 1.0 | 2.0 | 3.0 | 4.0
    private void Debug2DGrid(GameObject[,] array, string Identifier)
    {
        Debug.Log("Debug2DGrid Called");
        int invertedY = array.GetLength(1) - 1;
        int Line = 1;
        for (int y = invertedY; y >= 0; y--)
        {
            Debug.Log($"Array {Identifier} Line {Line}; " + ReturnforDebug(0, y, array) + ", " + ReturnforDebug(1, y, array) + ", " + ReturnforDebug(2, y, array) + ", " + ReturnforDebug(3, y, array) + ", " + ReturnforDebug(4, y, array));
            Line++;
        }
    }

    private string ReturnforDebug(int x, int y, GameObject[,] array)
    {
        if (array[x, y] != null)
        {
            return $"Room Name {array[x, y].name}; "+$"{x}"+". "+ $"{y}";
        }
        else { return "null"; }

...
public partial class GameDisplay : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        RegisterAnimationEvents();  // <= Isso pode dar um erro só em algumas versões do visual studeo. Antes de mudar checar se da erro na Unity.
    ...
    private void OnDisable()
    {
        ...
        handler.BulletSelected -= PlayerClickSound;
        handler.ZombieDamaged -= ShakeZombieVisual;
        UnregisterAnimationEvents();    // <= Isso pode dar um erro só em algumas versões do visual studeo. Antes de mudar checar se da erro na Unity.
    }
    ...
    private void HandleZombieDisplay()
    {
        ...
        if (cachedZombies.Count < handler.ZombieList.Count)
        {
            ...
            StartZombieAnimation(assignedDisplay);  // <= Isso pode dar um erro só em algumas versões do visual studeo. Antes de mudar checar se da erro na Unity.
        ...
        if (cachedZombies.Count > handler.ZombieList.Count)
        {
           ...
           ;
            ...
            StopZombieAnimation(assignedDisplay);   // <= Isso pode dar um erro só em algumas versões do visual studeo. Antes de mudar checar se da erro na Unity.
